...
INIT -------------------------------
 ...
 NAMESPACE DATA STRUCTURE --------------------------
   ...
   a namespace has the following fields in JSON:
     ...
     recipes               -> { name  -> recipe }
     alias_namespaces      -> { alias -> namespace filename3 }
     ...
     Namespace object with the following fields and methods
       id -> serialized number
       name -> "[namespace foo]"
       about -> "text what this namespace is about"
       filename   -> "filename"
       recipes    -> { name -> recipe object }
       aliasedNS  -> { name -> namespace object }
       fun        -> { name  -> function }
       data       -> { field -> value }
       bodyRecipe -> BodyRecipe object
       bodyInstance -> Instance object
       // methods 
       -> namespace(alias) -> return namespace object
       -> aliasNamespace( alias, namespace object ) -> attach alias to an other namespace object
       -> error( msg ) -> report and throw error
       -> findRecipe( htmlTagOrRecipeName ) -> returns Recipe Object if any is associated with that tag
       -> setup( node, filename ) builds up namespace object
//TODO       -> checkAndRefresh if data is updated calls refresh on 


      BodyRecipe Object fields and methods (beyond Recipe Object)
       head -> {
         script -> javascript text
         title  -> string
         style  -> css text
       ...
       -> installHead if data is updated calls refresh on 
...
const SP = window.SP ||= {};
{
  ...
  function init( fileSpaces, defaultFilename, attachPoint ) {
    ...
    Promise.resolve(bodyR.installHead())
      .then( () => {
        ...
        bodyInst = pageNS.bodyInstance = bodyR.createInstance(bodyR.rootBuilder);
  ...
  class Namespace extends Node {
    ...
    setup( node, filename ) {
      ...
      if (node.html) {
        ...
        ['preLoad', 'postLoad', 'on', 'data', 'listen', 'functions']
  ...
  class Recipe extends Node {
    ...
    setup( NS, recipeData, recipeName ) {
      ...
      [ 'attrs',    'data',    'listen',
        'postLoad', 'preLoad', 'when' ]
      .forEach( fld => ( this[fld] = recipeData[fld] ) );

      //console.log( this.functions, recipe.functions, recipeData.functions, "BURPH" );
      this.namedFillBuilders = {};
      this.contents = recipeData.contents; // used for prepping root builder. this may only contain one tagged thing
    ...
    prepRootBuilder() {
      ...
      const rootBuilder = this.rootBuilder = new ComponentBuilder();
      ...
      if (rootIsAlsoRecipe) {
        this.onTopOfBuilder = rootIsAlsoRecipe.prepRootBuilder();
      } else {
      ...
      // this node has fill_contents
      this.fill_contents = recipeRootNode.fill
  ...
  class ComponentBuilder extends Node {
    setup( node, recipe ) {
      this.tag = node.tag;
      this.name = `${node.tag} in ${recipe.name}`;
      this.recipe = recipe;
' call it named fill targets' ''
">>only if this doesn't overlay an other component builder"
      const el_B = this.elementBuilder = new ElementBuilder();
      el_B.setup( node, recipe );
    ...
    layer( node, recipe ) {
      ...
      this.recipe = recipe;
      const el_B = this.elementBuilder;

      // update this component builder
      [ 'when', 'fill_contents' ]
        .forEach( src => {
          const to = this[src];
          const from = node[src];
          Object.keys( from || {} )
            .forEach( fld => (to[fld] = from[fld]) );
        } );
      [ 'if', 'elseif', 'else', 'forvar', 'foreach' ]
        .forEach( fld => (fld in node && (this[fld] = node[fld])) );

      // update the element builder
      [ 'on', 'attrs' ]
        .forEach( src => {
          const to = el_B[src];
          const from = node[src];
    ...
    fillOut() {
      this.elementBuilder.fillOut();
    }

  }

  class ElementBuilder extends Node {
    setup( node, recipe ) {
      // copy because this may be modified by
      // recipe and instance building
      this.tag = node.tag;
      this.name = `${node.tag} in ${recipe.name}`;
      this.recipe = recipe;
      this.attrs = {};
  ...
  class Instance extends Node {
    ...
    layer( recipe, builder ) {
      ...
      const fun = this.fun;
      const postLoads = this.postLoads = [];
      ...
      [ recipe, builder ]
        .forEach ( from => {
          from.postLoad && postLoads.push( from.postLoad.bind(this) );
...
//      this._refresh_root_el( this.root_EL, this.instanceBuilder );
      ...
      if (! this.loaded) {
        this.postLoads.forEach( ol => ol(this) );
        this.loaded = true;
      }
    } //refresh
